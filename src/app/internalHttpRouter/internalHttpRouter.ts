...
export function createRouter<TEvent extends MinimalAPIGatewayProxyEvent>(routes: Route<TEvent>[]) {
  const routeMatchers = routes.map(matcherForRoute)

  return (event: TEvent): CicadaHandler<TEvent> => {
...
const notFoundRoute: CicadaHandler<MinimalAPIGatewayProxyEvent> = async () => notFoundHTMLResponse
