...
test('basic web test', async () => {
  ...
  expect(loginResponse.headers.get('Location')).toEqual(
    `https://github.com/login/oauth/authorize?client_id=${githubConfig.clientId}&redirect_uri=https://${webHostName}/github/auth/callback&scope=user:email&state=${githubConfig.githubCallbackState}`
  )
})
